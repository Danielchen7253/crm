...
AUTO_SYNC_FULL_SECONDS = float(os.getenv("CRM_AUTO_SYNC_FULL_SECONDS", "300"))
AUTO_SYNC_RECENT_LIMIT = int(os.getenv("CRM_AUTO_SYNC_RECENT_LIMIT", "3"))
AUTO_SYNC_RECENT_MESSAGES_LIMIT = int(os.getenv("CRM_AUTO_SYNC_RECENT_MESSAGES_LIMIT", "3"))
AUTO_SYNC_STATE = {"started": False, "last_ok": None, "last_error": None, "runs": 0, "imported": 0}
AUTO_SYNC_LAST_FULL = time.time()
CLOSED_TAG = "\u6210\u4ea4\u5ba2\u6237"
...
<style>:root{font-family:Arial,"Microsoft YaHei",sans-serif;color:#17202a;background:#f4f6f8}*{box-sizing:border-box}body{margin:0}.top{height:58px;background:#16202a;color:#fff;display:flex;align-items:center;gap:12px;padding:0 16px}.back{color:#fff;text-decoration:none;font-weight:800}.wrap{max-width:980px;margin:0 auto;padding:20px 16px;display:grid;gap:14px}.card{background:#fff;border:1px solid #d8dee8;border-radius:8px;padding:16px}h1{margin:0;font-size:18px}h2{margin:0 0 8px;font-size:16px}.muted{color:#6a7682;font-size:13px;line-height:1.45}.button{margin-top:12px;border:0;border-radius:8px;background:#1f8a70;color:#fff;font-weight:800;font-size:14px;padding:10px 14px;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;min-height:40px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}@media(max-width:700px){.grid{grid-template-columns:1fr}.wrap{padding:14px}.button{width:100%}}</style>
</head><body><header class="top"><a class="back" href="/">&lsaquo; CRM</a><h1>系统设置</h1></header><main class="wrap"><div class="card"><h2>系统管理</h2><div class="muted">这里会作为后续配置中心：固定回复、AI训练资料、自动回复开关、Meta / WhatsApp 接口状态、系统权限。</div></div><section class="grid"><div class="card"><h2>固定回复</h2><div class="muted">管理发货时间、提货地址、售后话术和 AI 固定回复规则。</div><a class="button" href="/?view=ai">打开AI话术</a></div><div class="card"><h2>AI 训练资料</h2><div class="muted">后面可以上传你的谈判话术、常见问题、产品资料，让 AI 学你的回复方式。</div></div><div class="card"><h2>渠道配置</h2><div class="muted">管理 Messenger、WhatsApp、Marketplace、Instagram、短信、Shopify 等接入状态。</div><a class="button" href="/?view=integrations">打开接口管理</a></div><div class="card"><h2>安全</h2><div class="muted">当前 CRM 首页和系统设置均已启用密码锁。</div></div></section></main></body></html>
"""
