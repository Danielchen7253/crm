...
def ensure_customer_from_messenger(psid):
    ...
    if identity:
        ...
        supabase_patch("customers", {"last_seen_at": now_iso(), "updated_at": now_iso()}, {"id": f"eq.{customer_id}"})
        return customer_id
    ...
    if selected_customer_id:
        ...
        if selected_customer:
            selected_messages = supabase_get(
                "messages",
                {
                    "customer_id": f"eq.{selected_customer_id}",
...
def dashboard():
    ...
    data = load_dashboard_data(request.args.get("customer"))
    customers, latest_by_customer, selected_customer, selected_messages, selected_customer_id = data
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .workspace { overflow: auto; padding: 24px; }
    .profile { max-width: 980px; margin: 0 auto; }
    .profile-head { display: flex; align-items: center; gap: 16px; padding-bottom: 18px; border-bottom: 1px solid #d8dee8; }
    ...
    .details { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 12px; margin-top: 18px; }
    ...
    .messages { display: flex; flex-direction: column; gap: 10px; max-width: 760px; }
    ...
    @media (max-width: 860px) { main { grid-template-columns: 1fr; height: auto; min-height: 0; } .list { border-right: 0; max-height: 46vh; } .workspace { padding: 18px; } .details { grid-template-columns: repeat(2, minmax(0, 1fr)); } .message { max-width: 92%; } }
    @media (max-width: 520px) { header { padding: 0 14px; } .details { grid-template-columns: 1fr; } .profile-head { align-items: flex-start; } h1 { font-size: 20px; } }
  </style>
</head>
<body>
...
<head>
  ...
  <style>
    ...
    main { max-width: 720px; width: 100%; background: white; border: 1px solid #d8dee8; border-radius: 8px; padding: 28px; }
...
<body>
  <main>
    <h1>CRM is online</h1>
    <p>The app is deployed. To start syncing Messenger customers, finish the database and Meta configuration in Render.</p>
    <ol>
      <li>Run <code>schema.sql</code> in the CRM Supabase project.</li>
      <li>Add <code>SUPABASE_URL</code> and <code>SUPABASE_SERVICE_ROLE_KEY</code> in Render.</li>
      <li>Add Messenger variables: <code>META_VERIFY_TOKEN</code>, <code>META_APP_SECRET</code>, <code>META_PAGE_ACCESS_TOKEN</code>, <code>META_PAGE_ID</code>.</li>
      <li>Use <code>/webhooks/meta</code> as the Meta callback path.</li>
    </ol>
