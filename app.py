...
AUTO_SYNC_STATE = {"started": False, "full_done": False, "last_ok": None, "last_error": None, "runs": 0}
...
def supabase_get_all(table, params=None, page_size=1000, max_pages=20):
    ...
    for page in range(max_pages):
        ...
        response = requests.get(
            ...
            headers=supabase_headers(range_header=f"{start}-{start + page_size - 1}"),
            params=params or {},
            timeout=20,
        )
        response.raise_for_status()
...
def graph_post(path, payload):
    response = requests.post(f"https://graph.facebook.com/{GRAPH_API_VERSION}/{path.lstrip('/')}", params={"access_token": META_PAGE_ACCESS_TOKEN}, json=payload, timeout=30)
    response.raise_for_status()
    return response.json()
...
def find_identity(provider, provider_user_id):
    rows = supabase_get("customer_identities", {"provider": f"eq.{provider}", "provider_user_id": f"eq.{provider_user_id}", "select": "customer_id,display_name,raw_profile", "limit": "1"})
    return rows[0] if rows else None


def find_identity_by_customer(customer_id, provider="messenger"):
    rows = supabase_get("customer_identities", {"customer_id": f"eq.{customer_id}", "provider": f"eq.{provider}", "select": "provider_user_id,display_name,raw_profile", "limit": "1"})
    return rows[0] if rows else None
...
def ensure_customer_from_messenger(psid, known_profile=None, touch=True, fetch_profile=False):
    profile = known_profile or {}
    if fetch_profile or not profile:
        profile = {**profile, **get_messenger_profile(psid)}
    ...
    if identity:
        ...
        supabase_patch("customer_identities", {"display_name": display_name, "raw_profile": profile, "updated_at": now_iso()}, {"provider": "eq.messenger", "provider_user_id": f"eq.{psid}"})
    ...
    supabase_post("customer_identities", {"customer_id": customer["id"], "provider": "messenger", "provider_user_id": psid, "display_name": display_name, "raw_profile": profile})
    return customer["id"]
...
def save_message(customer_id, provider_message_id, direction, text, attachments, raw_event, sent_at=None):
    ...
    supabase_post("messages", {"customer_id": customer_id, "provider": "messenger", "provider_message_id": provider_message_id, "direction": direction, "message_type": "attachment" if attachments else "text", "text": text, "attachments": attachments or [], "raw_event": raw_event, "sent_at": sent_at})
...
def process_messenger_event(event):
    ...
    customer_psid = event.get("recipient", {}).get("id") if direction == "outbound" else event.get("sender", {}).get("id")
    if not customer_psid:
        return
    customer_id = ensure_customer_from_messenger(customer_psid, touch=True, fetch_profile=True)
...
def sync_messenger_conversations(max_pages=1, conversation_limit=25, message_limit=5, fetch_profiles=False):
    ...
    for page in range(max_pages):
        if next_url:
            conversations = graph_get_url(next_url)
        else:
            conversations = graph_get(
                f"{META_PAGE_ID}/conversations",
                {"fields": f"participants{{id,name}},messages.limit({message_limit}){{id,message,from,to,created_time,attachments}}", "limit": str(conversation_limit)},
            )
        for conversation in conversations.get("data", []):
            ...
            psid = participant["id"]
            customer_id = ensure_customer_from_messenger(psid, {"name": participant.get("name")}, touch=False, fetch_profile=fetch_profiles)
...
def load_dashboard_data(selected_customer_id):
    customers = supabase_get_all("customers", {"select": "id,display_name,source,first_seen_at,last_seen_at,last_message_at,profile_pic_url,tags,locale,timezone,gender,metadata", "order": "last_seen_at.desc"})
    ...
    if selected_customer_id:
        ...
        if selected_customer:
            selected_messages = supabase_get("messages", {"customer_id": f"eq.{selected_customer_id}", "select": "direction,text,message_type,attachments,sent_at", "order": "sent_at.asc", "limit": "200"})
    ...
    while True:
        try:
            if is_database_configured() and META_PAGE_ID and META_PAGE_ACCESS_TOKEN:
                if not AUTO_SYNC_STATE["full_done"]:
                    sync_messenger_conversations(max_pages=20, conversation_limit=100, message_limit=10, fetch_profiles=False)
                    AUTO_SYNC_STATE["full_done"] = True
                else:
                    sync_messenger_conversations(max_pages=1, conversation_limit=25, message_limit=5, fetch_profiles=False)
...
def dashboard():
    ...
    return render_template_string(DASHBOARD_TEMPLATE, customers=customers, latest_by_customer=latest_by_customer, selected_customer=selected_customer, selected_messages=selected_messages, selected_customer_id=selected_customer_id)
...
def import_messenger_conversations():
    imported = sync_messenger_conversations(max_pages=20, conversation_limit=100, message_limit=10, fetch_profiles=False)
    return jsonify({"ok": True, "imported_messages": imported})


DASHBOARD_TEMPLATE = """
<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>CRM 客户工作台</title><style>:root{color-scheme:light;font-family:Arial,"Microsoft YaHei",sans-serif;background:#f5f6f8;color:#17202a}*{box-sizing:border-box}body{margin:0}header{height:56px;display:flex;align-items:center;justify-content:space-between;padding:0 20px;background:#fff;border-bottom:1px solid #d8dee8}.brand{font-weight:700;font-size:16px}.count{color:#5c6773;font-size:13px}main{display:grid;grid-template-columns:minmax(300px,380px) minmax(0,1fr);height:calc(100vh - 57px);min-height:620px}.list{background:#fff;border-right:1px solid #d8dee8;overflow:auto}.list-title{position:sticky;top:0;z-index:2;background:#fff;padding:14px 16px;border-bottom:1px solid #edf0f4;color:#5c6773;font-size:13px}.customer{display:grid;grid-template-columns:44px minmax(0,1fr);gap:12px;padding:14px 16px;border-bottom:1px solid #edf0f4;text-decoration:none;color:inherit;background:#fff}.customer:hover{background:#f8fafb}.customer.active{background:#eef7f4;border-left:4px solid #1f8a70;padding-left:12px}.avatar{width:44px;height:44px;border-radius:50%;background:#1f8a70;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;overflow:hidden;flex:none}.avatar.large{width:72px;height:72px;font-size:24px}.avatar img{width:100%;height:100%;object-fit:cover}.name{font-weight:700;margin-bottom:5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.preview{color:#3e4b57;font-size:13px;line-height:1.35;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:5px}.meta{color:#6a7682;font-size:12px;line-height:1.35}.workspace{display:flex;flex-direction:column;overflow:hidden}.profile{display:flex;flex-direction:column;min-height:0;height:100%}.profile-head{display:flex;align-items:center;gap:16px;padding:20px 24px 18px;border-bottom:1px solid #d8dee8;background:#fff}h1{margin:0 0 8px;font-size:24px;line-height:1.2}h2{margin:18px 0 12px;font-size:16px}.source{display:inline-flex;align-items:center;min-height:24px;padding:3px 9px;border:1px solid #c7d7d2;border-radius:999px;background:#eef7f4;color:#17634f;font-size:12px;font-weight:700;text-transform:capitalize}.details{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;padding:16px 24px;background:#f5f6f8}.field{background:#fff;border:1px solid #d8dee8;border-radius:8px;padding:12px;min-height:74px}.label{color:#6a7682;font-size:12px;margin-bottom:8px}.value{color:#17202a;font-size:14px;line-height:1.35;overflow-wrap:anywhere}.tags{display:flex;flex-wrap:wrap;gap:8px;padding:0 24px 14px;background:#f5f6f8}.tag{border:1px solid #d8dee8;border-radius:999px;background:#fff;color:#3e4b57;padding:5px 10px;font-size:12px}.conversation{flex:1;min-height:0;overflow:auto;padding:8px 24px 18px}.messages{display:flex;flex-direction:column;gap:10px;max-width:820px}.message{max-width:78%;padding:10px 12px;border-radius:8px;border:1px solid #d8dee8;background:#fff;line-height:1.45;font-size:14px;overflow-wrap:anywhere}.message.outbound{align-self:flex-end;background:#eaf2ff;border-color:#c9dcff}.message.inbound{align-self:flex-start}.message-time{color:#6a7682;font-size:11px;margin-top:6px}.reply{display:grid;grid-template-columns:minmax(0,1fr) 96px;gap:10px;padding:14px 24px;border-top:1px solid #d8dee8;background:#fff}textarea{width:100%;min-height:44px;max-height:120px;resize:vertical;border:1px solid #cfd7e2;border-radius:8px;padding:11px 12px;font:inherit;line-height:1.35}button{border:0;border-radius:8px;background:#1f8a70;color:#fff;font-weight:700;cursor:pointer}button:hover{background:#176f5a}.empty{max-width:620px;margin:24px;background:#fff;border:1px solid #d8dee8;border-radius:8px;padding:22px}@media(max-width:860px){main{grid-template-columns:1fr;height:auto;min-height:0}.list{border-right:0;max-height:42vh}.workspace{overflow:visible}.profile{height:auto}.details{grid-template-columns:repeat(2,minmax(0,1fr));padding:14px}.profile-head,.conversation,.reply,.tags{padding-left:14px;padding-right:14px}.message{max-width:92%}}@media(max-width:520px){header{padding:0 14px}.details{grid-template-columns:1fr}.profile-head{align-items:flex-start}h1{font-size:20px}.reply{grid-template-columns:1fr}button{min-height:42px}}</style></head><body><header><div class="brand">CRM 客户工作台</div><div class="count">{{ customers|length }} 个客户</div></header><main><section class="list"><div class="list-title">按最近互动时间排列，自动同步中</div>{% for customer in customers %}{% set latest = latest_by_customer.get(customer.id) %}<a class="customer {% if customer.id == selected_customer_id %}active{% endif %}" href="/?customer={{ customer.id }}"><div class="avatar">{% if customer.profile_pic_url %}<img src="{{ customer.profile_pic_url }}" alt="">{% else %}{{ (customer.display_name or 'C')[:1] }}{% endif %}</div><div><div class="name">{{ customer.display_name or '未命名客户' }}</div><div class="preview">{{ latest.text if latest and latest.text else '暂无文字消息' }}</div><div class="meta">{{ customer.source }} · {{ customer.last_seen_at }}</div></div></a>{% else %}<div class="customer"><div class="avatar">0</div><div><div class="name">还没有客户</div><div class="preview">客户同步后会自动显示在这里。</div></div></div>{% endfor %}</section><section class="workspace">{% if selected_customer %}<div class="profile"><div class="profile-head"><div class="avatar large">{% if selected_customer.profile_pic_url %}<img src="{{ selected_customer.profile_pic_url }}" alt="">{% else %}{{ (selected_customer.display_name or 'C')[:1] }}{% endif %}</div><div><h1>{{ selected_customer.display_name or '未命名客户' }}</h1><span class="source">{{ selected_customer.source }}</span></div></div><div class="details"><div class="field"><div class="label">第一次联系</div><div class="value">{{ selected_customer.first_seen_at or '-' }}</div></div><div class="field"><div class="label">最近互动</div><div class="value">{{ selected_customer.last_seen_at or '-' }}</div></div><div class="field"><div class="label">最近消息时间</div><div class="value">{{ selected_customer.last_message_at or '-' }}</div></div><div class="field"><div class="label">语言 / 时区</div><div class="value">{{ selected_customer.locale or '-' }}{% if selected_customer.timezone %} / {{ selected_customer.timezone }}{% endif %}</div></div></div><div class="tags">{% for tag in selected_customer.tags %}<span class="tag">{{ tag }}</span>{% else %}<span class="tag">暂无标签</span>{% endfor %}</div><div class="conversation"><h2>聊天记录</h2><div class="messages">{% for message in selected_messages %}<div class="message {{ message.direction }}"><div>{{ message.text if message.text else '[附件或系统消息]' }}</div><div class="message-time">{{ '客户发来' if message.direction == 'inbound' else '我们回复' }} · {{ message.sent_at }}</div></div>{% else %}<div class="empty">这个客户还没有可显示的聊天记录。</div>{% endfor %}</div></div><form class="reply" method="post" action="/customers/{{ selected_customer.id }}/messages"><textarea name="text" placeholder="输入要发给客户的消息" required></textarea><button type="submit">发送</button></form></div>{% else %}<div class="empty"><h2>请选择客户</h2><p>左侧客户会按最近互动时间自动排序。点击客户后，这里会显示客户档案和聊天记录。</p></div>{% endif %}</section></main></body></html>
"""
