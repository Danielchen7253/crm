...
CATEGORY_COLOR_CLASSES = {
    "商用冰箱维修客户": "cat-commercial-fridge",
    "家用冰箱维修客户": "cat-home-fridge",
    "空调维修客户": "cat-ac-customer",
    "空调维修师傅": "cat-ac-tech",
    "批发商客户": "cat-wholesale",
}
...
def tag_color_class(tag):
    return CATEGORY_COLOR_CLASSES.get(tag, "")


def colored_customer_tags(customer):
    return [{"name": tag, "class_name": tag_color_class(tag)} for tag in customer_tags(customer)]
...
def enrich_customers(customers):
    for customer in customers:
        ...
        customer["colored_tags"] = colored_customer_tags(customer)
        customer["category_class"] = next((tag["class_name"] for tag in customer["colored_tags"] if tag["class_name"]), "")
...
{% endfor %}
{% else %}
<div class="middle-head"><div class="title-row"><div class="middle-title">{{ view_title }}</div>{% if view == 'customers' %}<a class="mini-button" href="/?view=manual">新建客户</a>{% endif %}</div><div class="middle-sub">{{ customer_pool|length }} / {{ customers|length }} · 按最后互动排序</div></div>
{% for customer in customer_pool %}
<a class="customer {{ customer.category_class }} {% if customer.id == selected_customer_id %}active{% endif %}" href="/?view={{ view }}&customer={{ customer.id }}" title="{{ customer.display_name or '未命名客户' }}">
...
<div class="panel">
<h2>客户基本资料</h2>
<form class="manual-form" method="post" action="/admin/customers/manual">
...
{% for category in customer_categories %}<div class="field"><div class="label">{{ loop.index }}</div><div class="value"><span class="pill tag {{ tag_color_class(category) }}">{{ category }}</span></div></div>{% endfor %}
</aside>
</section>
{% elif view == 'ai' %}
